...
Determined that using more than 50 clusters causes memory issues - have different hierarchical models by region of the world. Start with EU + UK + Northern Ireland countries:
```{r}
...
```{r}
EUCountries = list("Austria", "Belgium", "Bulgaria", "Croatia", "Cyprus", "Czechia", "Denmark", "Estonia", "Finland", "France", "Germany", "Greece", "Hungary", "Ireland", "Italy", "Latvia", "Lithuania", "Luxembourg", "Malta", "Netherlands", "Poland", "Portugal", "Romania", "Slovakia", "Slovenia", "Spain", "Sweden","United Kingdom of Great Britain and Northern Ireland")

EUCountriesPresent = intersect(EUCountries,unique(lifeData2$Country))
```


```{r}
#collect new lifeData based on EU countries
lifeData = data.frame(matrix(ncol = 22, nrow = 0))
colnames(lifeData) = colnames(lifeData2)
#get four data points from each country in countriesGreaterEqualThree
for (i in 1:length(EUCountries)) {
  country = EUCountriesPresent[i]
  #get all rows from lifeData2 where $Country==country
  relevantRows = lifeData2[lifeData2$Country==country,]
  #get top four rows
  #datapoints = relevantRows[1:4,]
  lifeData = rbind(lifeData,relevantRows)
}
```
...
life_expectancy.model = function()
{
  for (i in 1:N) {
     ...
     mu[indices[i],i] <- 
          ...
          + betay[indices[i],17]*mortality[i] #mortality #started at 17
